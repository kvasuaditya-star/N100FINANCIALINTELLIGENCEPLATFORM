...
def test_opm_mismatch(caplog):
    ...
    with caplog.at_level(logging.WARNING):
        val = operating_profit_margin(25, 100, source_opm=20, company_id="TESTCO", year="2024")
        assert val == 25.0
        assert any("OPM Cross-Check Mismatch for TESTCO in 2024" in r.message for r in caplog.records)
...
def test_roce_and_roa():
    ...
    assert return_on_capital_employed(15, 5, 40, 40, 20) == 20.0  # (15+5)/(40+40+20)*100
...
def test_capital_allocation_pattern():
    ...
    assert classify_capital_allocation(100, -50, -30, 80) == "Shareholder Returns"  # (+,-,-) and high CFO/PAT
    assert classify_capital_allocation(100, -50, -30, 200) == "Reinvestor"          # (+,-,-) and low CFO/PAT
