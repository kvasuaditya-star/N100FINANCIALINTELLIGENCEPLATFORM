...
def run_sector_reports():
    ...
    comp_df = pd.read_sql_query(
        ...
        "FROM companies c JOIN sectors s ON c.id = s.company_id", conn
    )
    ...
    for sector in broad_sectors:
        ...
        elements.append(Paragraph(f"Number of Constituents: {len(sector_comps)}", STYLE_NORMAL))
        elements.append(Spacer(1, 15))
        
        # Table data
        table_data = [["Company ID", "Company Name", "CFO Quality", "CapEx Intensity", "Capital Allocation"]]
