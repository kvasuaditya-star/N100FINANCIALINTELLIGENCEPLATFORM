...
def run_portfolio_summary():
    ...
    doc = SimpleDocTemplate(out_path, pagesize=A4, leftMargin=50, rightMargin=50, topMargin=50, bottomMargin=50)
    elements = []
    ...
    elements.append(Paragraph(f"Total Companies Monitored: {len(comp_df)}", STYLE_NORMAL))
    
    if not intel_df.empty:
        elements.append(Paragraph("Cash Flow Intelligence Aggregates", STYLE_H2))
        
        cfo_dist = intel_df["cfo_quality_label"].value_counts()
        elements.append(Paragraph(f"High Quality CFO: {cfo_dist.get('High Quality', 0)} companies", STYLE_NORMAL))
        elements.append(Paragraph(f"Accrual Risk: {cfo_dist.get('Accrual Risk', 0)} companies", STYLE_NORMAL))
        
        capex_dist = intel_df["capex_label"].value_counts()
        elements.append(Paragraph(f"Asset Light: {capex_dist.get('Asset Light', 0)} companies", STYLE_NORMAL))
        elements.append(Paragraph(f"Capital Intensive: {capex_dist.get('Capital Intensive', 0)} companies", STYLE_NORMAL))
        
        distress = intel_df[intel_df["distress_flag"] == True]
        elements.append(Paragraph(f"Companies showing Distress Signals: {len(distress)}", STYLE_NORMAL))
        
        deleverage = intel_df[intel_df["deleveraging_flag"] == True]
        elements.append(Paragraph(f"Companies actively Deleveraging: {len(deleverage)}", STYLE_NORMAL))
