...
def compute_composite_score(df, df_cf):
    ...
    # CFO / PAT
    df['cfo_pat_ratio'] = np.where(df['net_profit'] > 0, df['cash_from_operations_cr'] / df['net_profit'], 0)
    ...
    for _, row in df.iterrows():
        ...
        try:
            ...
            end_cf = df_cf[(df_cf['company_id'] == cid) & (df_cf['year'].str.startswith(str(year_num)))]
            start_cf = df_cf[(df_cf['company_id'] == cid) & (df_cf['year'].str.startswith(start_year))]
            
            if not end_cf.empty and not start_cf.empty:
                end_fcf = end_cf['operating_activity'].values[0] + end_cf['investing_activity'].values[0]
                start_fcf = start_cf['operating_activity'].values[0] + start_cf['investing_activity'].values[0]
    ...
    metrics = [
        'return_on_equity_pct', 'roce', 'net_profit_margin_pct',
        'fcf_cagr_5yr', 'cfo_pat_ratio', 'fcf_positive_flag',
    ...
    df['interest_coverage'] = np.where(df['icr_label'] == 'Debt Free', 100.0, df['interest_coverage'])
    ...
    for sector in df['broad_sector'].unique():
        ...
        prof_score = scores['return_on_equity_pct'] * 0.15 + scores['roce'] * 0.10 + scores['net_profit_margin_pct'] * 0.10
        
        # Cash Quality (30%): FCF CAGR 15%, CFO/PAT 10%, FCF flag 5%
        cash_score = scores['fcf_cagr_5yr'] * 0.15 + scores['cfo_pat_ratio'] * 0.10 + scores['fcf_positive_flag'] * 0.05
        ...
        if total_score.max() > total_score.min():
            total_score = (total_score - total_score.min()) / (total_score.max() - total_score.min()) * 100.0
...
def apply_filters(df, df_fr_hist, filters):
    ...
    for key, value in filters.items():
        if key == 'roe_min':
            ...
            mask = (filtered['debt_to_equity'] <= value) | (filtered['broad_sector'] == 'Financials')
            ...
            for _, row in filtered.iterrows():
                ...
                try:
                    ...
                    prev_de = df_fr_hist[(df_fr_hist['company_id'] == cid) & (df_fr_hist['year'].str.startswith(prev_year))]['debt_to_equity']
...
def generate_screener_output(db_path="data/nifty100.db"):
    ...
    df_fr_hist = pd.read_sql_query("SELECT company_id, year, debt_to_equity FROM financial_ratios", conn)
    ...
    kpi_columns = [
        ...
        'return_on_equity_pct', 'roce', 'debt_to_equity', 'free_cash_flow_cr',
        'revenue_cagr_5yr', 'pat_cagr_5yr', 'net_profit_margin_pct', 'pe_ratio',
        'pb_ratio', 'dividend_yield_pct', 'dividend_payout_ratio_pct',
        'interest_coverage', 'asset_turnover', 'sales', 'market_cap_crore',
        'revenue_cagr_3yr'
    ]
    ...
    with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
        for preset_id, preset_data in presets.items():
            ...
            export_df['composite_quality_score'] = export_df['composite_quality_score'].round(2)
    ...
    for preset_id, preset_data in presets.items():
        ...
        col_map = {col[0].value: col[0].column for col in ws.iter_cols(min_row=1, max_row=1)}
            ...
            for filter_key, threshold in filters.items():
                ...
                try:
                    ...
                    if filter_key.endswith('_min'):
                        ...
                        if filter_key == 'de_max' and ws.cell(row=row_idx, column=col_map['broad_sector']).value == 'Financials':
                            passed = True
                        else:
                            passed = val <= threshold
