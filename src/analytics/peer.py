...
def generate_peer_comparison_report(df, df_percentiles):
        ...
        for pg in peer_groups:
            ...
            for col, metric_name in metrics.items():
                ...
                for cid in out_df['company_id']:
                    rank_val = df_percentiles[(df_percentiles['company_id'] == cid) & (df_percentiles['metric'] == metric_name)]['percentile_rank'].values
            ...
            summary_row = {'company_id': 'SUMMARY', 'company_name': 'Peer Group Median', 'is_benchmark': 0}
            for col, metric_name in metrics.items():
    ...
    for pg in peer_groups:
        ...
        for row in range(2, ws.max_row + 1):
            if benchmark_col_idx and ws.cell(row=row, column=benchmark_col_idx).value == 1:
...
def generate_radar_charts(df, df_percentiles):
    ...
    nifty_avgs = {k: df[k].median() for k in metrics.keys()}
    ...
    for _, row in df.iterrows():
        ...
        # Get company values
        comp_vals = [row.get(k, 0) if not pd.isna(row.get(k, 0)) else 0 for k in metrics.keys()]
        
        # Get reference values
        if pd.isna(pg):
            ref_vals = [nifty_avgs[k] for k in metrics.keys()]
            ref_label = "Nifty 100 Median"
        else:
            pg_df = df[df['peer_group_name'] == pg]
            ref_vals = [pg_df[k].median() if not pd.isna(pg_df[k].median()) else 0 for k in metrics.keys()]
        ...
        ax.plot(angles, normalized_ref, color='orange', linewidth=2, linestyle='dashed', label=ref_label)
