...
def main():
    ...
    df_latest['PE_vs_sector_median_pct'] = np.where(
        ...
        ((df_latest['pe_ratio'] - df_latest['sector_median_pe']) / df_latest['sector_median_pe']) * 100.0,
        np.nan
    )
    ...
    df_summary = df_latest[summary_cols].rename(columns={
        'pe_ratio': 'P/E',
        'pb_ratio': 'P/B',
        'ev_ebitda': 'EV/EBITDA'
    })
    ...
    df_summary['PE_vs_sector_median_pct'] = df_summary['PE_vs_sector_median_pct'].round(2)
    ...
    print(f"Valuation summary excel generated at: {summary_excel_path} ({len(df_summary)} companies)")
    ...
    df_flags_export = df_flags[[
        'company_id', 'company_name', 'sector', 'pe_ratio', 'sector_median_pe',
        'PE_vs_sector_median_pct', 'flag', 'FCF_yield_pct', '5yr_median_PE'
    ]].rename(columns={
        'pe_ratio': 'P/E',
    ...
    df_flags_export['P/E vs Sector Median %'] = df_flags_export['P/E vs Sector Median %'].round(2)
    ...
    print(f"Valuation flags CSV generated at: {flags_csv_path} ({len(df_flags_export)} companies)")
