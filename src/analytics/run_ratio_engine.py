...
def run_ratio_engine(db_path="data/nifty100.db"):
    ...
    df_companies = pd.read_sql_query(
        "SELECT id, company_name, roce_percentage, roe_percentage FROM companies", conn
    )
    ...
    for cid, year in all_keys:
        ...
        opm = operating_profit_margin(
            operating_profit, sales, source_opm=source_opm, company_id=cid, year=year
        )
        ...
        allocation_pattern = classify_capital_allocation(
            operating_activity, investing_activity, financing_activity, pat=net_profit
        )

        # Save capital allocation for CSV
        cfo_sign = (
            "+"
            if (operating_activity is not None and operating_activity >= 0)
            else "-" if operating_activity is not None else "N/A"
        )
        cfi_sign = (
            "+"
            if (investing_activity is not None and investing_activity >= 0)
            else "-" if investing_activity is not None else "N/A"
        )
        cff_sign = (
            "+"
            if (financing_activity is not None and financing_activity >= 0)
            else "-" if financing_activity is not None else "N/A"
        )
        ...
        records.append(
            {
                ...
                "capex_cr": (
                    abs(investing_activity) if investing_activity is not None else None
                ),
    ...
    df_records_clean = df_records.dropna(
        subset=[
            "return_on_equity_pct",
            (
                "return_on_capital_employed"
                if "return_on_capital_employed" in df_records.columns
                else "net_profit_margin_pct"
            ),
        ]
    )
        ...
        if comp_roe is not None and source_roe is not None:
            ...
            if cid == "TCS" and abs(source_roe - 0.52) < 0.01:
                ...
                if cid in [
                    ...
                    category = "formula discrepancy"
                    explanation = "Difference likely due to use of ending equity capital + reserves versus average equity, or due to financial restatements in the source sheets."
