...
if df_latest.empty:
    ...
    df_chart = df_sec.dropna(
        subset=["sales", "return_on_equity_pct", "market_cap_crore"]
    )

    if df_chart.empty:
        st.warning(
            f"No company metrics available in {selected_sector} to plot bubble chart."
        )
    else:
        st.subheader(f"Bubble Map of {selected_sector} Companies")
        st.markdown(
            "*Bubble size corresponds to **Market Capitalization**. Bubble color corresponds to **Sub-Sector**.*"
        )
    ...
    sector_medians = (
        df_latest.groupby("broad_sector")[
            [
