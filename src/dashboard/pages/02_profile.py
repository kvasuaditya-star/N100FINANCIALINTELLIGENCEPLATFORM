...
from utils.db import apply_custom_style, get_companies, get_ratios, get_pl, get_bs, get_connection
        ...
        if co_row.empty:
            ...
            for _, r in df_pl.iterrows():
                ...
                if not bs_yr.empty:
                    ...
                    borrowings = bs_yr.iloc[0]['borrowings'] if bs_yr.iloc[0]['borrowings'] is not None else 0
                    
                    ebit = (pbt if pbt is not None else 0) + interest
                    capital_employed = (equity_cap if equity_cap is not None else 0) + (reserves if reserves is not None else 0) + borrowings
            ...
            if not df_ratios_sorted.empty:
                ...
                kpis = [
                    ...
                    ("Net Profit Margin", f"{val_npm:.2f}%" if not pd.isna(val_npm) else "N/A"),
                    ("D/E Ratio", f"{val_de:.2f}" if not pd.isna(val_de) else "N/A"),
                    ("Revenue CAGR 5yr", f"{val_cagr:.2f}%" if not pd.isna(val_cagr) else "N/A"),
                    ("FCF (Latest Year)", f"₹{val_fcf:.1f} Cr" if not pd.isna(val_fcf) else "N/A")
                ]
            ...
            with chart_col1:
                ...
                if not df_pl.empty:
                    ...
                    fig_bar.update_layout(
                        ...
                        yaxis=dict(gridcolor='rgba(255,255,255,0.05)', title="Amount (Cr)"),
            ...
            with chart_col2:
                ...
                if not df_ratios_chrono.empty:
                    ...
                    fig_line.update_yaxes(title_text="ROE (%)", secondary_y=False, gridcolor='rgba(255,255,255,0.05)')
            ...
            conn = get_connection()
            c = conn.cursor()
            c.execute("SELECT pros, cons FROM prosandcons WHERE company_id = ?", (ticker,))
            pc_row = c.fetchone()
            conn.close()
            ...
            with pro_col:
                st.markdown("<h4 style='color: #26a69a;'>✔️ Key Strengths (Pros)</h4>", unsafe_allow_html=True)
                if pc_row and pc_row[0]:
                    pros_list = [p.strip().lstrip('-').strip() for p in pc_row[0].split('\n') if p.strip()]
                    for pro in pros_list:
                        st.markdown(f'<div class="pro-badge">✔️ {pro}</div>', unsafe_allow_html=True)
                else:
                    st.write("No positive key metrics highlighted.")
                    
            with con_col:
                st.markdown("<h4 style='color: #ef5350;'>❌ Risk Areas (Cons)</h4>", unsafe_allow_html=True)
                if pc_row and pc_row[1]:
                    cons_list = [c.strip().lstrip('-').strip() for c in pc_row[1].split('\n') if c.strip()]
                    for con in cons_list:
                        st.markdown(f'<div class="con-badge">❌ {con}</div>', unsafe_allow_html=True)
                else:
