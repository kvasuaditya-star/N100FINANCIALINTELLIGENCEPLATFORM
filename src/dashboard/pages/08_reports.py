    ...
    if selected_option:
        ...
        if df_docs.empty:
            ...
            for _, row in df_docs.iterrows():
                ...
                with cols[1]:
                    if url:
                        st.markdown(f"<code style='color: #00e5ff; font-size: 0.85rem;'>{url}</code>", unsafe_allow_html=True)
                    else:
                        st.write("No Link")
                        
                with cols[2]:
                    if not url:
                        st.markdown('<span style="background-color: rgba(239, 83, 80, 0.2); color: #ef5350; padding: 4px 10px; border-radius: 4px; border: 1px solid rgba(239, 83, 80, 0.4); font-size: 0.85rem; font-weight: 600;">Report unavailable</span>', unsafe_allow_html=True)
                    elif url_is_valid:
                        st.markdown(f'<a href="{url}" target="_blank" style="background-color: rgba(0, 229, 255, 0.2); color: #00e5ff; padding: 6px 12px; border-radius: 6px; border: 1px solid rgba(0, 229, 255, 0.4); font-size: 0.9rem; font-weight: 600; text-decoration: none; text-align: center; display: inline-block;">📥 Download Report</a>', unsafe_allow_html=True)
                    else:
                        st.markdown('<span style="background-color: rgba(239, 83, 80, 0.2); color: #ef5350; padding: 4px 10px; border-radius: 4px; border: 1px solid rgba(239, 83, 80, 0.4); font-size: 0.85rem; font-weight: 600;">Report unavailable</span>', unsafe_allow_html=True)
