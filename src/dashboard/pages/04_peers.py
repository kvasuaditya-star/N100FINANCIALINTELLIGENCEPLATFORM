...
conn = get_connection()
df_pg_list = pd.read_sql_query("SELECT DISTINCT peer_group_name FROM peer_groups WHERE peer_group_name IS NOT NULL AND peer_group_name != 'No peer group assigned'", conn)
conn.close()
...
if df_pg_list.empty:
    ...
    conn = get_connection()
    df_group_mapping = pd.read_sql_query("SELECT company_id, is_benchmark FROM peer_groups WHERE peer_group_name = ?", conn, params=(selected_group,))
    conn.close()
    ...
    if df_group_mapping.empty or df_latest.empty:
        ...
        group_df = pd.merge(df_group_mapping, df_latest, left_on='company_id', right_on='company_id', how='inner')
        
        # 2. Select Company from Group
        company_names = {row['company_id']: f"{row['company_id']} - {row['company_name']}" for _, row in group_df.iterrows()}
        selected_ticker = st.selectbox("Select Target Company to Analyze", list(company_names.keys()), format_func=lambda x: company_names[x])
        ...
        comp_vals = [target_row.get(k, 0) if not pd.isna(target_row.get(k, 0)) else 0 for k in metrics_map.keys()]
        group_medians = [group_df[k].median() if not pd.isna(group_df[k].median()) else 0 for k in metrics_map.keys()]
        ...
        st.subheader(f"📊 Radar Comparison: {selected_ticker} vs Peer Median (Percentile Basis)")
        ...
        table_df['Benchmark'] = table_df['is_benchmark'].apply(lambda x: "Yes" if x == 1 else "No")
        
        display_cols = [
            'company_id', 'company_name', 'Benchmark', 'Composite', 
            'ROE %', 'ROCE %', 'NPM %', 'D/E', 'FCF (Cr)', 'PAT CAGR 5y %', 'Rev CAGR 5y %'
        ]
        
        df_display = table_df[display_cols].rename(columns={
            'company_id': 'Ticker',
        ...
        def highlight_benchmark_row(row):
            ...
            for val in row:
                if is_benchmark:
                    styles.append('background-color: rgba(255, 215, 0, 0.15); border: 1.5px solid #ffd700;')
                elif is_selected:
                    styles.append('background-color: rgba(0, 229, 255, 0.1); font-weight: bold;')
                else:
                    styles.append('')
            return styles
