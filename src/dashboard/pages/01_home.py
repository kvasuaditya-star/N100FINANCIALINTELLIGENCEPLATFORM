...
selected_year = st.sidebar.selectbox(
    "Select Year", list(range(2024, 2018, -1)), index=0
)
...
if df.empty:
    ...
    kpis = [
        ...
        (
            "Average ROE",
            f"{avg_roe:.2f}%" if not pd.isna(avg_roe) else "N/A",
            "Return on Equity",
        ),
        (
            "Median P/E",
            f"{median_pe:.2f}" if not pd.isna(median_pe) else "N/A",
            "Price to Earnings",
        ),
        (
            "Median D/E",
            f"{median_de:.2f}" if not pd.isna(median_de) else "N/A",
            "Debt to Equity",
        ),
        (
            "Median Rev CAGR (5y)",
            f"{median_rev_cagr:.2f}%" if not pd.isna(median_rev_cagr) else "N/A",
            "Top-line compound growth",
        ),
    ...
    with right_col:
        ...
        top_5["ROE %"] = top_5["return_on_equity_pct"].apply(
            lambda x: f"{x:.2f}%" if not pd.isna(x) else "N/A"
        )
        top_5["D/E"] = top_5["debt_to_equity"].apply(
            lambda x: f"{x:.2f}" if not pd.isna(x) else "N/A"
        )
        top_5["FCF (Cr)"] = top_5["free_cash_flow_cr"].apply(
            lambda x: f"₹{x:,.2f}" if not pd.isna(x) else "N/A"
        )
