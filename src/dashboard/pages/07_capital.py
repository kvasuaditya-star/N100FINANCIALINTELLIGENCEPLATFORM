...
# Set page config
st.set_page_config(
    page_title="Nifty 100 Analytics - Capital Allocation Map", layout="wide"
)
...
if df_latest.empty:
    ...
    df_latest["size_weight"] = (
        df_latest["market_cap_crore"] if size_option == "Market Cap" else 1.0
    )
    # Handle NaN values for weights
    df_latest["size_weight"] = (
        df_latest["size_weight"].fillna(1.0).apply(lambda x: max(x, 1.0))
    )
    ...
    selected_pattern = st.selectbox(
        "Select Capital Allocation Pattern to view companies:", available_patterns
    )

    df_pattern_list = df_latest[df_latest["allocation_pattern"] == selected_pattern]

    st.markdown(
        f"Found **{len(df_pattern_list)}** companies in the **{selected_pattern}** pattern category:"
    )

    if not df_pattern_list.empty:
        # Formatted Columns
        df_pattern_list["Market Cap (Cr)"] = df_pattern_list["market_cap_crore"].apply(
            lambda x: f"₹{x:,.1f}" if not pd.isna(x) else "N/A"
        )
        df_pattern_list["CFO (Cr)"] = df_pattern_list["operating_activity"].apply(
            lambda x: f"₹{x:,.1f}" if not pd.isna(x) else "N/A"
        )
        df_pattern_list["CFI (Cr)"] = df_pattern_list["investing_activity"].apply(
            lambda x: f"₹{x:,.1f}" if not pd.isna(x) else "N/A"
        )
        df_pattern_list["CFF (Cr)"] = df_pattern_list["financing_activity"].apply(
            lambda x: f"₹{x:,.1f}" if not pd.isna(x) else "N/A"
        )
        df_pattern_list["Net Profit (Cr)"] = df_pattern_list["net_profit"].apply(
            lambda x: f"₹{x:,.1f}" if not pd.isna(x) else "N/A"
        )
        ...
        st.dataframe(
            df_pattern_list[display_cols].rename(
                columns={
                    "company_id": "Ticker",
