...
fcf_min_val = st.sidebar.slider(
    "FCF Min (Cr)", -1000.0, 5000.0, key="fcf_min", step=50.0
)
rev_cagr_val = st.sidebar.slider(
    "Revenue CAGR 5yr Min (%)", -30.0, 100.0, key="rev_cagr", step=1.0
)
pat_cagr_val = st.sidebar.slider(
    "PAT CAGR 5yr Min (%)", -30.0, 100.0, key="pat_cagr", step=1.0
)
...
div_yield_val = st.sidebar.slider(
    "Dividend Yield Min (%)", 0.0, 10.0, key="div_yield", step=0.1
)
icr_min_val = st.sidebar.slider(
    "Interest Coverage Min", 0.0, 100.0, key="icr_min", step=1.0
)
...
if df.empty:
    ...
    filtered["interest_coverage_computed"] = np.where(
        filtered["icr_label"] == "Debt Free", 1000.0, filtered["interest_coverage"]
    )
    ...
    filtered = filtered[
        ...
        (
            (filtered["debt_to_equity"].fillna(999) <= de_max_val)
    ...
    if filtered.empty:
        st.info(
            "No companies match the current filter criteria. Adjust the sliders in the sidebar."
        )
    else:
        ...
        filtered["ICR"] = filtered["interest_coverage"].apply(
