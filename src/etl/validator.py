...
class DataValidator:
    def __init__(self):
        self.failures = (
            []
        )  # list of dict: {company_id, year, field, rule_id, issue, severity}
        self.info_counters = {}
    ...
    def validate_dq_06_positive_sales(self, df_pl, df_sectors):
        ...
        if not df_sectors.empty:
            fin_df = df_sectors[
                df_sectors["broad_sector"]
                .str.lower()
                .str.contains("financial", na=False)
    ...
    def save_failures(self, filepath):
        ...
        if not self.failures:
            ...
            df = pd.DataFrame(
                columns=["company_id", "year", "field", "rule_id", "issue", "severity"]
            )
        else:
