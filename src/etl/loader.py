...
def load_data():
    ...
    df_co["company_name"] = (
        df_co["company_name"].astype(str).str.replace("\n", " ").str.strip()
    )
    ...
    if not is_valid_pk:
        raise ValueError(
            "CRITICAL: Companies table PK uniqueness check failed (DQ-01). Halting load."
        )
    ...
    log_audit(
        ...
        len(df_co_raw),
        len(df_co_insert),
        len(df_co_raw) - len(df_co_insert),
        time.time() - t0,
    )
    ...
    log_audit(
        ...
        len(df_sec_raw),
        len(df_sec_insert),
        len(df_sec_raw) - len(df_sec_insert),
        time.time() - t0,
    )
    ...
    df_pl = validator.validate_dq_03_fk_integrity(
        "profitandloss", df_pl, valid_companies
    )
    ...
    log_audit(
        ...
        len(df_pl_raw),
        len(df_pl_insert),
        len(df_pl_raw) - len(df_pl_insert),
        time.time() - t0,
    )
    ...
    df_bs = validator.validate_dq_03_fk_integrity(
        "balancesheet", df_bs, valid_companies
    )
    ...
    log_audit(
        ...
        len(df_bs_raw),
        len(df_bs_insert),
        len(df_bs_raw) - len(df_bs_insert),
        time.time() - t0,
    )
    ...
    log_audit(
        ...
        len(df_cf_raw),
        len(df_cf_insert),
        len(df_cf_raw) - len(df_cf_insert),
        time.time() - t0,
    )
    ...
    log_audit(
        ...
        len(df_an_raw),
        len(df_an_insert),
        len(df_an_raw) - len(df_an_insert),
        time.time() - t0,
    )
    ...
    df_doc["Year"] = (
        pd.to_numeric(df_doc["Year"], errors="coerce").fillna(0).astype(int)
    )
    ...
    df_doc_insert = df_doc[cols].rename(
    ...
    log_audit(
        ...
        len(df_doc_raw),
        len(df_doc_insert),
        len(df_doc_raw) - len(df_doc_insert),
        time.time() - t0,
    )
    ...
    log_audit(
        ...
        len(df_pc_raw),
        len(df_pc_insert),
        len(df_pc_raw) - len(df_pc_insert),
        time.time() - t0,
    )
    ...
    # Date validation
    invalid_dates = (
        ~df_pr["date"].astype(str).str.match(r"^\d{4}-\d{2}-\d{2}$", na=False)
    )
    ...
    df_pr = validator.validate_dq_03_fk_integrity(
        "stock_prices", df_pr, valid_companies
    )
    ...
    log_audit(
        ...
        len(df_pr_raw),
        len(df_pr_insert),
        len(df_pr_raw) - len(df_pr_insert),
        time.time() - t0,
    )
    ...
    log_audit(
        ...
        len(df_mc_raw),
        len(df_mc_insert),
        len(df_mc_raw) - len(df_mc_insert),
        time.time() - t0,
    )
    ...
    df_rat = validator.validate_dq_03_fk_integrity(
        "financial_ratios", df_rat, valid_companies
    )
    ...
    log_audit(
        ...
        len(df_rat_raw),
        len(df_rat_insert),
        len(df_rat_raw) - len(df_rat_insert),
        time.time() - t0,
    )
    ...
    log_audit(
        ...
        len(df_pg_raw),
        len(df_pg_insert),
        len(df_pg_raw) - len(df_pg_insert),
        time.time() - t0,
    )
    ...
    print(
        f"Saved load audit log with {len(audit_log)} entries to output/load_audit.csv"
    )
